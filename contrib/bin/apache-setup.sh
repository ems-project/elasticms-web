...
for filename in /opt/secrets/*; do
...
EOL
    ...
    <If "%{REQUEST_URI} =~ m#bundles/emsform/manifest\.json\$#">
      Header set Access-Control-Allow-Origin "*"
      Header set Access-Control-Allow-Methods "GET"
      Header set Access-Control-Allow-Headers "application/json"
    </If>
...
EOL
    ...
    if [ -z ${APACHE_ENVIRONMENTS+x} ]; then
        ...
        source /opt/bin/apache-setup-one-alias.sh
    else
        source /opt/bin/apache-setup-multi-alias.sh
    fi
